...
router.post("poder.estrella", "/estrella", async(ctx) => {
    try {
        ...
        const jugador = await ctx.orm.Jugador.increment({estrellas: 1}, {where:{id: ctx.request.body.jugadorId}});
        ctx.body = jugador;
        ctx.status = 201;
    } catch (error) {
        ctx.body = error;
...
// Recibe el id del jugador y de su victima (obtenido gracias al GET anterior), intercambian posiciones
router.post("poder.swap", "/swap", async(ctx) => {
    try {
        ...
        await ctx.orm.Jugador.update({posicion: posVictima}, {where: {id: ctx.request.body.jugadorId}});
        await ctx.orm.Jugador.update({posicion: posJugador}, {where: {id: ctx.request.body.victimaId}});

        const jugadorNuevo = await ctx.orm.Jugador.findOne({where: {id: ctx.request.body.jugadorId}});
        const victimaNueva = await ctx.orm.Jugador.findOne({where: {id: ctx.request.body.victimaId}});
...
// Recibe el id del jugador, el id de su victima (obtenido gracias al GET) y el tipo de dado que desea robar
router.post("poder.robo", "/robar", async(ctx) => {
    try {
        ...
        const jugador = await ctx.orm.Jugador.increment(dataUpdate, {where:{id: ctx.request.body.jugadorId}});
        const victima = await ctx.orm.Jugador.decrement(dataUpdate, {where:{id: ctx.request.body.victimaId}});
